 ...
 */
/**
 * Западный знак Солнца против ведического — и разница между ними в градусах.
 *
 * Это первое, обо что спотыкается читательница. Она всю жизнь знает себя
 * Водолеем, открывает наш расчёт и видит Козерога. Первая мысль — не
 * «интересно», а «у них калькулятор сломан», и человек уходит, не дочитав.
 *
 * Причина не в ошибке, а в точке отсчёта. Западная астрология считает от точки
 * весеннего равноденствия, индийская — от неподвижных звёзд. Из-за прецессии
 * (медленного колебания земной оси) эти начала разошлись примерно на 24° и
 * продолжают расходиться на ~50″ в год. Почти градус знака в год человеческой
 * жизни, но за две тысячи лет накопился почти целый знак — поэтому ведический
 * знак Солнца обычно оказывается предыдущим.
 */
export interface SignShift {
  /** Знак Солнца по западному (тропическому) зодиаку. */
  western: RashiName;
  /** Он же по ведическому (сидерическому). */
  vedic: RashiName;
  /** Айянамша — накопленное расхождение начал отсчёта, в градусах. */
  ayanamsaDeg: number;
  /** Совпали ли знаки: у части людей Солнце далеко от границы и знак тот же. */
  same: boolean;
}

export function calcSignShift(person: Person): SignShift {
  const utc = birthMoment(person);
  const tropical = sunLongitude(utc);
  const sidereal = toSidereal(tropical, utc);
  const w = rashiName(rashiIndex(tropical));
  const v = rashiName(rashiIndex(sidereal));
  return {
    western: w,
    vedic: v,
    ayanamsaDeg: Math.round((((tropical - sidereal) % 360) + 360) % 360 * 10) / 10,
    same: w === v,
  };
}
