...
// Градиент глубины, как и в блоке про притяжение. Шесть секций подряд с
// одинаковой рамкой из пяти частей читаются как шаблон: глаз перестаёт
// различать блоки и начинает скользить. Две главные идут полностью, остальные
// в сжатой форме — со сценой и разделением «навсегда / лечится», без
// объяснения центра и разбора своих активаций.
//
// Сжатая форма нужна не только для ритма: в PDF сворачивания не существует, и
// без неё туда уехала бы вся стена целиком. На странице у сжатых блоков будет
// кнопка «показать полностью» — но это надстройка, а не замена.
const condOrder = ranked.filter((x) => x.kind === "conditioningBare" || x.kind === "conditioningAnchored");
const condItems = condOrder.map((h) => {
  const side = h.side as "a" | "b";
  const data = side === "a" ? pro.a : pro.b;
  return { side, item: data.conditioning.find((c) => c.center === h.center)! };
});
...
const condRest = condItems.slice(COND_DEEP);
if (condRest.length) {
  h2(`Ещё ${condRest.length} ${plural(condRest.length, "место", "места", "мест")}, где вы влияете друг на друга`);
  p("Здесь то же самое, только короче. Если какая-то тема попадёт в больное — на странице её можно развернуть целиком.", "");
  for (const { side, item: c } of condRest) {
    const t = CONDITIONING_BY_CENTER[c.center];
    const s = side === "a" ? t.you : t.partner;
    p(`• ${CENTER_NAMES[c.center]} — ${t.topic} (открыт ${side === "a" ? "у тебя" : "у партнёра"})`,
      `  ${g(s.scene)}`,
      `  Не изменится: ${g(s.fixed)}`,
      `  Изменится: ${g(s.changeable)}`,
      "");
  }
}
